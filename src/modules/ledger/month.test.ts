...
describe('месяц, который ещё идёт (Р-22, Р-24)', () => {
  ...
  // по записанному: иначе четыре дня, которых в базе нет, идут днями
  // без трат и занижают расход (Р-24).
  it('записанный срок отличается от прожитого, и оба числа известны', () => {
    expect(monthShare('2026-09', '2026-09-22', '2026-09-18')).toEqual({
      passed: 18,
      elapsed: 22,
      total: 30,
    })
  ...
  it('записи доведены дальше этого месяца — месяц записан целиком', () => {
    expect(monthShare('2026-09', '2026-09-22', '2026-10-05')).toEqual({
      passed: 22,
      elapsed: 22,
      total: 30,
    })
  ...
  it('записей за месяц нет вовсе — записано ноль дней, а не все', () => {
    expect(monthShare('2026-09', '2026-09-22', '2026-07-30')).toEqual({
      passed: 0,
      elapsed: 22,
      total: 30,
    })
  ...
  // Прошедший месяц, у которого записи обрываются на середине, полным
  // не считается: сравнивать его с обычным целиком нельзя.
  it('прошедший месяц с оборванными записями остаётся неполным', () => {
    expect(monthShare('2026-08', '2026-09-22', '2026-08-18')).toEqual({
      passed: 18,
      elapsed: 31,
      total: 31,
    })
  ...
  it('отчёт месяца говорит, идёт ли месяц, только когда его спросили о дне', () => {
    ...
    expect(monthReport(data(list), '2026-09', '2026-09-22').running).toEqual({
      passed: 22,
      elapsed: 22,
      total: 30,
    })
